# -*- coding: utf-8 -*-
'for generating Fig 4C data, axonJT(0.9)'
...
vaxJT = h.Vector().record(axonJT(0.9)._ref_v)
vd1 = h.Vector().record(daughter1(0.3)._ref_v)
s_vecJT = h.Vector().record(axonJT(0.9)._ref_s_hhy) #record the s-gate before the branching point 
...
for g in [120]: #range(65,125,5): 
    ...
    pd.DataFrame({"t":t,"vAIS(0.5)":vAIS,"v_ax(0.007)":vstartax,"v_axJT(0.9)":vaxJT,"v_d1(0.3)":vd1,"s-gate_axJT(0.9)":s_vecJT,"s-gate_d1(0.3)":s_vecd1}).to_csv(f"GR={GR}_4C_JT09.csv", index = False, sep="\t")
    pd.DataFrame({"Monitored Location (um)":MonitoredLocations, "Spike Times":SpikeTimesToSave}).to_csv(f"SpikeTimes_GR=_{GR}_a{axon.diam}_d1_{daughter1.diam}_d2_{daughter2.diam}_4C_JT09.csv", index = False, sep="\t")
    print("\a")
